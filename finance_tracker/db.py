        ...
        with self.connect() as con:
            ...
            for item in transactions:
                if item["unsupported_currency"]:
                    rejected += 1
                try:
                    con.execute("""INSERT INTO transactions(source_file_id,booking_date,amount_cents,currency,merchant,description,account,external_id,transaction_kind,raw_json,fingerprint,category_id,category_reason,excluded_reason,unsupported_currency,created_at)
                    VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", (
                        source_id, item["booking_date"], item["amount_cents"], item["currency"], item["merchant"], item["description"], item["account"],
                        item["external_id"], item["transaction_kind"], json.dumps(item["raw"], ensure_ascii=False), item["fingerprint"], item["category_id"],
